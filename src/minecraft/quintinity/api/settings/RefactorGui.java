	...
	public void tickStart(EnumSet<TickType> type, Object... tickData) 
	{
		if (type.equals(EnumSet.of(TickType.CLIENT))) {
			if (minecraft.currentScreen != null && minecraft.currentScreen instanceof GuiOptions) {
				...
				if (!SettingsAPI.guiAPIinstalled && list.size() < 13 && SettingsAPI.getInstance().buttons.size() > 0) {
				...
				else if (SettingsAPI.guiAPIinstalled && list.size() < 14 && SettingsAPI.getInstance().buttons.size() > 0) {
					for (int i = 0; i < list.size(); i ++) {
						if (list.get(i) instanceof GuiButton) {
							GuiButton gbutton = (GuiButton) list.get(i);
							if (gbutton.id == 200) {
